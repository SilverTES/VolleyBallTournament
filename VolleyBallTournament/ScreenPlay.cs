...
namespace VolleyBallTournament
{
    internal class ScreenPlay : Node
    {
        ...
        public ScreenPlay()
        {
            ...
            {
                _matchs[i] = (Match)new Match($"Terrain {i + 1}").AppendTo(this);
            ...
            {
                _groups[i] = (Group)new Group($"Pool {i}").AppendTo(this);
        ...
        public override Node Draw(SpriteBatch batch, GameTime gameTime, int indexLayer)
        {
            ...
            if (indexLayer == (int)Layers.Main)
            {
                ...
                batch.Draw(Static.TexBG00, Vector2.Zero, Color.White);
                batch.FillRectangle(AbsRectF, Color.Black * .5f);

                //batch.Grid(Vector2.Zero, Screen.Width, Screen.Height, 40, 40, Color.Gray * .5f, 1f);

                batch.CenterBorderedStringXY(Static.FontMain2, $"{_tournamentName}", AbsRectF.TopCenter + Vector2.UnitY * 32, Color.White, Color.Black);
            }
